...
def create_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    if db_user:
        raise HTTPException(status_code=400, detail="Электронная почта уже существует")
...
def create_booking(booking: BookingCreate, user_id: int, db: Session = Depends(get_db)):
    ...
    if conflicts:
        raise HTTPException(status_code=400, detail="Временной интервал уже забронирован")
